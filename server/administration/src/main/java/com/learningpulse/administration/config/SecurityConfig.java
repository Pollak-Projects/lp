...
import com.learningpulse.administration.config.JwtConverter;
import com.learningpulse.administration.config.KeycloakJwt;
...
public class SecurityConfig {
    ...
    AuthoritiesConverter authoritiesConverter() {
        return jwt -> {
            Optional<Map<String, Object>> realmAccess = Optional.ofNullable((Map<String, Object>) jwt.getClaims().get("realm_access"));
            Stream<String> realmRoles = realmAccess
                    .map(map -> (List<String>) map.get("roles"))
                    .stream().flatMap(Collection::stream);

            Stream<String> resourceAccessRoles = Optional.ofNullable((Map<String, Object>) jwt.getClaims().get("resource_access"))
                    .map(Map::values)
                    .stream()
