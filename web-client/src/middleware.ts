...
const publicPages = ["/login", "/forbidden", "/register"];
const defaultPathnameRegex = RegExp(
  `^/((api|_next/static|_next/image|favicon.ico|sitemap.xml|robots.txt|${publicPages.flatMap((p) => p.slice(1)).join("|")}).*)$`,
  "i"
);
...
export default auth((req: NextAuthRequest) => {
  ...
  if (!session && !defaultPathnameRegex.test(req.nextUrl.pathname)) return NextResponse.redirect(new URL("/login", req.url));
